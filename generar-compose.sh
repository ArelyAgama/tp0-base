...
# Validar que la cantidad de clientes sea un número
if ! [[ "$CANTIDAD_CLIENTES" =~ ^[0-9]+$ ]]; then
    echo "Error: La cantidad de clientes debe ser un número"
    exit 1
fi

# Validar que la cantidad de clientes sea mayor a 0
if [ "$CANTIDAD_CLIENTES" -eq 0 ]; then
    echo "Error: La cantidad de clientes debe ser mayor a 0"
    exit 1
fi
